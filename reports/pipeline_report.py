...
def render_markdown(proposals: list):
    ...
    table = Table(title="Proposal Engine Proposal Pipeline Report", show_lines=True)
...
def render_excel(proposals: list, output_path: str = "reports/pipeline_report.xlsx"):
    ...
    write_sheet(ws_summary, proposals, "Proposal Engine Proposal Pipeline — All BUs")
    ...
    for bu, bu_proposals in sorted(bu_groups.items()):
        ...
        write_sheet(ws_bu, bu_proposals, f"Proposal Engine Pipeline — {bu}")
...
def main(fmt, bu, region, output_path):
    """Proposal Engine Pipeline Report — scan outputs and generate report."""
    console.print(f"[bold cyan]Proposal Engine Pipeline Report[/bold cyan] — format: {fmt}")
    if bu:
