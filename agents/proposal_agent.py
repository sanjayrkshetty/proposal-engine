...
EXEC_SUMMARY_PROMPT = """You are a senior SISA Information Security proposal writer.
...
and position SISA as the trusted partner. Be professional and concise.
...
class ProposalAgent(BaseAgent):
    def run(self, brief_data: dict, scope_data: dict, client_info: dict,
            ...
            proposal_number: str = "SISA-0001") -> dict:
        ...
        # Generate or use provided exec summary
        ...
        if not billing_contact:
            billing_contact = "billing@sisainfosec.com | +91-80-4040-5050"
        ...
        # Build scope context from questionnaire if available
    ...
    def _generate_exec_summary(self, brief_data: dict, scope_data: dict,
        ...
        if exec_summary_context and len(exec_summary_context) > 50:
            # Use provided context as base but enrich
            ...
            return (
                ...
                f"SISA Information Security proposes a {tier}-tier {self.config.get('name', 'security engagement')} "
    ...
    def _default_template(self) -> str:
        return """# SISA Information Security
...
SISA Information Security is a global payment and cybersecurity company, specialising in forensics, compliance, and managed security services. Founded in 2012 and headquartered in Bengaluru, India, SISA has protected over 1,000 organisations across 40+ countries. SISA is the only forensics company in Asia-Pacific accredited as both a PCI Forensic Investigator (PFI) and a QSA company by the PCI Security Standards Council. SISA's team of 200+ certified security professionals holds GCFA, GCIH, CREST, CISSP, and CISM credentials, delivering world-class security outcomes for financial institutions, healthcare organisations, and critical infrastructure operators.
...
4. SISA retains the right to withdraw or amend this proposal if material information is found to be inaccurate.
...
7. Data handling: All client data processed during the engagement is handled per SISA's ISO 27001-certified data handling procedures.
8. Governing law: This agreement is governed by the laws of India unless a separate jurisdiction is agreed.
