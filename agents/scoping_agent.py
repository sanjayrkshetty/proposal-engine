...
{{
  ...
  "assumptions": ["list of 3-5 key assumptions"],
  "indicative_price_range": {{"low": number, "high": number, "currency": "INR"}}
}}
...
class ScopingAgent(BaseAgent):
    ...
    def _to_markdown(self, data: dict, client_info: dict, tier: str) -> str:
        ...
        lines = [
            ...
            "",
            f"**Indicative Price:** ₹{p.get('low', 0):,} – ₹{p.get('high', 0):,} {p.get('currency', 'INR')}",
            "",
            "## In Scope",
            ...
            *[f"- {d}" for d in data.get("deliverables", [])],
            "",
            "## Assumptions",
            *[f"- {a}" for a in data.get("assumptions", [])],
            "",
            "## SLA Commitments",
